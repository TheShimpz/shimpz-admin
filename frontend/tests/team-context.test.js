...
test('composer context provides model buttons and complete Assistant scope controls', () => {
  ...
  assert.doesNotMatch(contextControlsSource, /selectOnlyTeamAssistant\(assistant\.id\)|copy\.onlyThis/);
  ...
  assert.match(contextControlsSource, /\{@const selected = \$teamContext\.selectedAssistantIds\.includes\(assistant\.id\)\}[\s\S]*class:selected/);
  assert.match(contextControlsSource, /<input\s+class="sr-only"\s+type="checkbox"/);
  assert.match(contextControlsSource, /class="selection-mark" class:visible=\{selected\}[\s\S]*<svg[\s\S]*<path/);
  assert.match(contextControlsSource, /\.assistant-choices \{[\s\S]*border-block: 1px solid var\(--border-strong\);[\s\S]*padding: 0\.45rem 0;/);
  assert.match(contextControlsSource, /\.assistant-choice \{[^}]*padding: 0 var\(--dialog-pad\);/);
  assert.match(contextControlsSource, /\.assistant-choice\.selected \{ background: rgba\(0, 240, 255, 0\.065\); \}/);
  assert.doesNotMatch(contextControlsSource, /\.assistant-choice:focus-within \{[^}]*background:/);
