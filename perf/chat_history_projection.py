...
CASES = (
    ("empty", 0, None),
    ("eight_short", 8, None),
    ("large_history", 10_000, None),
    ("long_text", 8, LONG_TEXT),
    ("long_nfc", 8, LONG_NFC_TEXT),
    ("long_ascii", 8, LONG_ASCII_TEXT),
)
...
def _case(path: Path, name: str, eligible: int, long_text: str | None) -> dict[str, object]:
    store.STORE_PATH = path
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="shimpz-history-perf-") as temporary:
            ...
            results = [
                _case(root / f"{name}.sqlite3", name, eligible, long_text) for name, eligible, long_text in CASES
            ]
